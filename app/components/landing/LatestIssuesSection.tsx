...
export default function LatestIssuesSection({ locale, dir, issues = [] }: LatestIssuesSectionProps) {
  const t = translations[locale];
  const latestIssues = Array.isArray(issues) ? issues : [];
    ...
    <section className="rounded-2xl bg-white px-6 py-12 shadow-sm ring-1 ring-slate-200 sm:px-10">
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-3">
        {latestIssues.map((issue) => (
          <article key={issue.id} className="rounded-xl border border-slate-200 bg-slate-50 p-5">
            {issue.Volume?.name ? (
              <p className={`text-xs font-semibold uppercase tracking-wide text-accent ${dir === "rtl" ? "text-end" : "text-start"}`}>
                {issue.Volume.name}
              </p>
            ) : null}
            <h3 className={`mt-3 text-lg font-semibold text-slate-800 ${dir === "rtl" ? "text-end" : "text-start"}`}>{issue.name}</h3>
            <p
              className={`mt-2 text-sm text-slate-600 ${dir === "rtl" ? "text-end" : "text-start"} ${containsArabic(issue.description) ? "font-arabic text-end" : ""}`}
              dir={containsArabic(issue.description) ? "rtl" : "ltr"}
              lang={containsArabic(issue.description) ? "ar" : "en"}
            >
              {issue.description}
            </p>
            <Link
